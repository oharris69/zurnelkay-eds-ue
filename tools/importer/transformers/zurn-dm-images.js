...
/**
 * Transformer: zurn Dynamic Media / Scene7 image handling.
 *
 * The source uses Scene7 IS/Image URLs (e.g.
 * https://zurnelkay.scene7.com/is/image/Elkay/3NL-GASKET?$ZE_PRODUCT_MAIN$),
 * confirmed via migration-work/metadata.json .images.mapping keys.
 *
 * Converts DM/S7 <img> tags into anchors so the URL round-trips through
 * markdown intact; a companion auto-block in scripts/scripts.js rebuilds
 * responsive <picture> markup at render time.
 *
 * Runs in afterTransform ONLY: block parsers run between the hooks and
 * extract <img> references into block cells (cards/carousel/columns), so
 * rewriting imgs in beforeTransform would empty those cells.
 *
 * Helpers below are copied from the canonical dm-scene7-helpers.js and must
 * stay byte-identical to it — do not re-derive the detection regex.
 */
...
export default function transform(hookName, element, payload) {
  ...
  element.querySelectorAll('img').forEach((img) => {
    ...
    // via linkTextToAlt() so screen readers skip decorative images.
